 ...
 */
@Component
public class WtNotificationProcessor implements NotificationProcessor {

    private static final String MSG = "✔️ Акции с сигналом по Wave trend: \n\n";
        ...
        if (indicators.iterator().hasNext() && indicators.iterator().next() instanceof ProcessedIndicators) {
            return Pair.of(
                MSG, indicators.stream()
                        ...
                        .filter(stocks -> StringUtils.isNotBlank(stocks.getWtTrend()))
                        .filter(stocks -> !stocks.getWtTrend().equals(ProcessingResult.RsiSign.NO_SIGN.name()))
